...
\begin{itemize}
    ...
    \item \textbf{Errori leggibili e localizzati:} il server risponde con codici d'errore stabili, che il client traduce nella lingua scelta (IT/EN); così, se i dati di mercato non sono disponibili o si inserisce un valore sbagliato, l'utente vede un messaggio chiaro invece di un errore tecnico.
\end{itemize}
...
Gli aggiornamenti più recenti, tra un po' di pulizia e qualche aggiunta:

\begin{itemize}[itemsep=-2pt]
    \item Il trascinamento e il ridimensionamento dei widget sono diventati direttive riusabili, al posto di codice ripetuto in ogni componente.
    \item Inventario e connection hub ora si ridimensionano davvero (i contenuti scalano con la dimensione) e l'orologio non va più a capo.
    \item È stato tolto un po' di codice inutilizzato e uniformati alcuni nomi.
    \item I messaggi di errore del server sono ora codici stabili tradotti dal client, così seguono la lingua scelta.
    \item È stato aggiunto il braccialetto DeQ nel profilo, sbloccabile con il Premium e il profilo completo.
\end{itemize}